...
import { View,StyleSheet,Text,ImageBackground,TextInput,StatusBar,Platform,TouchableOpacity } from 'react-native'
  ...
  const Item = ({ item, onPress, style }) => (
    <View style={{backgroundColor:'rgba(0,0,0,0.45)',borderRadius:10,marginHorizontal:10,marginTop:25}}>
        <View style={{paddingHorizontal:15}}>
            <Text style={{color:'white',marginTop:10,fontSize:20}}>{item.title}</Text>
            <Text style={{color:'white',marginTop:10,fontSize:15}}>{item.text}</Text>
            <Text style={{color:'white',marginTop:15,marginBottom:20,fontSize:14}}>{new Date(item.date).toDateString() }</Text>
        </View>
